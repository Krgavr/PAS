...
pie(rc, col=2:7, main="Relativni cetnosti studijnich oboru", labels=popis)
...
#frekvencni rozdeleni pro ciselnou diskretnipromennou 
ac <- table(passengers)
kas <- cumsum(ac)
...
data.frame(cbind("n(i)"=ac, "N(i)"=kas, "f(i)"=rc, "F(i)"=krc))

#frekvencni polygon 
#извлекаем названия стообуов из переменой ac и преобразуем в числовой формат 
x.val <- as.numeric(as.character(names(ac)))
# извлекаем значения из переменной ас 
ac.n <- as.numeric(ac)

#строим граф, состоящий только из столбцов 
plot(x.val, ac.n, type ="h", lwd =3, col="green", main="Frekvencni polygon", xlab ="Pocet spolucestujicich", ylab = "Absolutni cetnost")
# создаем доп. значения 
x.val2 <- min(x.val):max(x.val) # создаем вектор со всеми отсутствующими значениями из x.val
ac2 <- rep(0, length(x.val2)) # создаем вектор нулей, длиной x.val2
# прописываем цикл, который изменяет вектор нулей на нормальные значения 
for(i in 1:length(x.val2)){
...
# рисуем динию6 которая закончит фреквенчни полигон 
lines(x.val2, ac2, col='red')
...
delky <- Cars93$Length
summary(delky)
hist(delka, col="skyblue", border='black', main='Histogram delky aut',xlab ="delky aut", ylab ='absolutni cetnost')
# Min - nejkratší auto
# Max - nejdelší auto
...
delka <- Cars93$Length
hist(delka, col='blue', main='Hist delky vozu', xlab = 'delka vozu', ylab='absolutni cetnost', breaks=20)
summary(delka)
var(delka)
sd(delka)
