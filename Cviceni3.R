...
vaha <- Policie$weight
#hiatogram 
hist(vaha, col='skyblue', main ='hist vahy ', xlab = 'vaha', ylab='ASOLUTNI CETNOST')
# krabicovy graf 
boxplot(vaha, main='boxpot vahy', ylab ='vaha')
#variability 
#royptyl 
var(vaha) 
# smerodatna odchylka 
sd(vaha)
#variacni koeficent 
CoefVar(vaha) # 14% - nizka variabilita. vetsina hodnot vahy jsou kolem prumeru
# mezikvartilove rozpeti 
IQR(vaha) # 17.15 male mezikvarilove roypeti, oznacuje, ze 50 % hodnot ma skoro stejnou vahu v rozsahu od 69.7 do 86.9
#popisne statisticke polohy 
summary(vaha)
# tvar rozdeleni 
#sikmost 
Skew(vaha) # 0.29 - kladna sikmost, pravostrane rozdeleni, ale blizke k simetrickemu 
#spicatost 
Kurt(vaha) # -0.84 - zaporna spicatost, plohe/partikurticke roydeleni. 
...
sd(puls); sd(tlak) # puls ma vetsi smerodatnou odchylku 
IQR(puls); IQR(tlak) # 50 % hodnot pulzu se nahazi na vetsim rozsahu 
CoefVar(puls); CoefVar(tlak) # puls ma vetsi variabilitu 
...
reacni_doba <- Policie$react
#hist
oldpar <- par(mfrow = c(1,2))
hist(reacni_doba, col='skyblue', main='Histogram reakcni doby', xlab ='reakcni doba', ylab ='absolutni cetnost')
boxplot(reacni_doba, main = 'Boxplot reakcni doby')
...
react_doba <- Policie$react
jadro_react_doba <- density(react_doba)
hist(react_doba, col='skyblue', main='Hist react doba', xlab = 'absolutni cetnos', ylab = 'reakcni doba', freq =F)
lines(jadro_react_doba)
jadro_react_doba$x[which.max(jadro_react_doba$y)]
## Najdete modus(y) pro vahu
vaha<- Policie$weight
jadro_vaha <- density(vaha)  
hist(vaha, col='skyblue', main= 'Histogram', xlab ='vaha', ylab ='absolutni cetnost', freq=F, breaks=5)
lines(jadro_vaha)
...
tlak = Policie$diast
hist(tlak, col='skyblue', main='histogram', xlab='tlak', ylab ='absolutni cetnost')
A <- 70
h <- 5
cetnosti<-hist(tlak, plot=F)$counts
d0 <-cetnosti[6]-cetnosti[5]
d1 <-cetnosti[6]-cetnosti[7]
modus<- A + h*d0/(d0+d1)
(modus <- A + h*d0/(d0+d1))
###########################
## Odlehla pozorovani
