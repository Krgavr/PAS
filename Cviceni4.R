...
hist(vyska, col='skyblue', main='histogrem vysky', xlab = 'vyska', ylab='hustota', freq=F)
jadro_vyska <- density(vyska)
lines(jadro_vyska)
jadro_vyska$x[which.max(jadro_vyska$y)]
...
bmi <- Stulong$bmi
hist(bmi, col='skyblue', main='Histogram bmi', xlab = 'bmi', ylab ='hustota',breaks=20, freq=F)
jadro_bmi <- density(bmi, bw=10)
lines(jadro_bmi)
jadro_bmi <- density(bmi, bw=3)
lines(jadro_bmi, col=4)
jadro_bmi <- density(bmi)
lines(jadro_bmi, col=2)
jadro_bmi$x[which.max(jadro_bmi$y)]

# pracujte s vahou jako se spojitou promennou a najdete modus
vaha<- Stulong$vaha
hist(vaha, col='skyblue', main='histogram vaha', xlab = 'vaha', ylab='hustota',breaks=20,  freq=F)
jadro_vaha<- density(vaha, bw=5)
lines(jadro_vaha)
...
syst1<- Stulong$syst1
hist(syst1, col='skyblue', main='HISTOGRAM syst1')
meze <- hist(syst1, plot=F)$breaks
pocet <- hist(syst1, plot=F)$counts
m <- which.max(pocet)
A<- meze[m]
h <- meze[m]-meze[m-1]
d0<- pocet[m]-pocet[m-1]
d1<- pocet[m]-pocet[m+1]
modus<- A+h*d0/(d0+d1)
# Urcete modus pro cukr
  # pokud sousedni sloupec k tomu nejvyssimu chybi, bere se jeho cetnost jako 0
cukr<- Stulong$cukr
hist(cukr,col='skyblue', main="Histogram cukr", xlab='cukr', ylab = 'absolutni cetnost')
meze <- hist(cukr, plot=F)$breaks
pocet <- hist(cukr, plot=F)$counts
m <- which.max(pocet)
A <- meze[m]
h <- meze[2]-meze[1]
d0 <- pocet[m]-0
...
vaha <- Stulong$vaha
boxplot(vaha, col='yellow', border='orange3', main='Krabicovy graf', ylab='vaha v kg')
...
HM
vaha[vaha>HM] # -adepti na odlehle hodnoty 
...
#################################################
  ...
  # Kladná špičatost (leptokurtic)
  data_high_kurtosis <- c(rnorm(90), rnorm(10, mean=5, sd=1))  # Přidáme extrémní hodnoty
  qqnorm(data_high_kurtosis)
  qqline(data_high_kurtosis, col="red", lwd=2)
  
  # Záporná špičatost (platykurtic)
  data_low_kurtosis <- runif(100, min=-2, max=2)  # Rovnoměrné rozdělení
  qqnorm(data_low_kurtosis)
  qqline(data_low_kurtosis, col="red", lwd=2)
