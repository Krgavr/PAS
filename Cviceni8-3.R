...
#b Kolik pokusů můžeme očekávat do prvního zjištění osoby mužského pohlaví?
1/0.4882
...
#d P(X > x) = 0.75 => p(x<=x) = 0.25
qnorm(0.25, mu, sigma)


# e) P(a<X<b)=0.95
# вероятность, что значения находятся вне этого интервала = 0.05 => что 0.025 с одно строны и 0.025 с другой
qnorm(0.025, 23, 5)
qnorm(0.975, 23, 5)
...
#b) P(50<=x<=100) => p(x<=100) - p(x<= 50)
plnorm(100, mi, sigma) - plnorm(50, mi, sigma)
